...
export default function BattlePage() {
  ...
  useEffect(() => {
    const unsubRoom = subscribeRoom(code, (r) => {
      if (!r) { toast("Room not found.", "error"); router.push("/battle"); return }
      setRoom(r)
      if (r.status === "playing") {
        setQuestionStartMs(r.questionStartMs)
        setSelected(null)
        setTimeLeft(15)
      }
  ...
  }, [room?.status, room?.currentQ, code])

  // Auto-advance if time runs out
  useEffect(() => {
    if (room?.status !== "playing" || !room) return
    if (timeLeft > 0) return

    const advance = async () => {
      const nextQ = room.currentQ + 1
      await goToNextQuestion(code, nextQ, room.questions.length)
    }
    advance()
  }, [timeLeft, room?.status, room?.currentQ, code])
  ...
  const handleSelect = useCallback(async (idx: number) => {
    if (selected !== null || !room || room.status !== "playing") return
    setSelected(idx)

    const now = Date.now()
    const timeMs = now - questionStartMs
    const currentQ = room.questions[room.currentQ]
    const correct = idx === currentQ.answer

    await submitAnswer(code, user!.uid, room.currentQ, idx, timeMs, correct)

    // Auto advance after all players (or 2s)
    setTimeout(async () => {
      const nextQ = room.currentQ + 1
      await goToNextQuestion(code, nextQ, room.questions.length)
    }, 1500)
  }, [selected, room, code, user, questionStartMs])

  const handleFinish = useCallback(async () => {
    // Calculate rating
    const scores = players.map((p) => p.score)
    const myScore = players.find((p) => p.uid === user!.uid)?.score || 0
    ...
    // Cleanup room after results
    setTimeout(() => deleteRoom(code), 60000)
  }, [players, code, user])

  useEffect(() => {
    if (room?.status === "finished" && resultPlayers.length === 0) {
      handleFinish()
    }
  }, [room?.status, handleFinish, resultPlayers.length])
    ...
    <div className="min-h-screen bg-ink">
      ...
      <div className="relative mx-auto max-w-5xl px-4 py-6 md:py-10">
        ...
        <div className="flex items-center justify-between mb-6">
          <Link href="/battle" className="flex items-center gap-2 text-caption font-bold uppercase tracking-wider text-canvas-soft/40 hover:text-primary transition-colors">
        ...
        {room.status === "waiting" && (
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-body-sm text-canvas-soft/30 mb-8 max-w-sm">
              Share this code with friends. Host can start when everyone is in.
            </p>
            ...
            {players.length === 0 && (
              <p className="text-body-sm text-canvas-soft/30 mb-8">No players yet...</p>
            )}

            {user.uid === room.hostId && (
              <Button variant="primary" size="lg" onClick={handleStart} className="cursor-pointer px-10">
                Start Battle <ChevronRight className="w-5 h-5 ml-1" />
              </Button>
            )}
            {user.uid !== room.hostId && (
              <p className="text-body-sm text-primary/60 animate-pulse">Waiting for host to start...</p>
            )}
          </div>
        )}
            ...
            <div className="lg:col-span-3">
              ...
              <div className="flex items-center justify-between mb-4">
                <span className="text-caption text-canvas-soft/30 uppercase tracking-wider">
                  Question {room.currentQ + 1} of {totalQs}
                </span>
                <div className="flex items-center gap-2">
                  <Timer className="w-4 h-4" style={{ color: timeLeft > 5 ? "#9fe870" : "#d03238" }} />
                  <span className="text-body-md-strong font-mono" style={{ color: timeLeft > 5 ? "#9fe870" : "#d03238" }}>
                    {timeLeft}s
                  </span>
                </div>
              </div>
              <div className="h-1.5 bg-surface-elevated rounded-full overflow-hidden mb-6">
                <div className="h-full rounded-full transition-all" style={{ width: `${((room.currentQ + (selected !== null ? 0.5 : 0)) / totalQs) * 100}%`, background: timeLeft > 5 ? "#9fe870" : "#d03238" }} />
              </div>
              ...
              <div className="rounded-2xl bg-surface-card border border-primary/5 p-6 md:p-8">
                ...
                <div className="space-y-3">
                  {currentQ.options.map((opt, oi) => {
                    ...
                    else if (selected !== null && isCorrect) cls = "border-primary/30 bg-primary/5 text-primary/70"
                    else if (selected !== null) cls = "border-surface-elevated bg-ink text-canvas-soft/30"
                    return (
                      <button key={oi} disabled={selected !== null} onClick={() => handleSelect(oi)}
                        ...
                        <span className={`inline-flex items-center justify-center w-8 h-8 rounded-full text-sm font-semibold mr-3 ${
                          selected !== null && isCorrect ? "bg-primary text-on-primary" :
        ...
        {room.status === "finished" && (
          <div className="flex flex-col items-center py-8 text-center">
            ...
            <div className="w-full max-w-md space-y-2 mb-8">
              {resultPlayers.length > 0 ? resultPlayers.map((p, i) => {
                      ...
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="text-left min-w-0">
                          ...
                          <p className="text-caption text-canvas-soft/30">
                            {p.answers.filter((a) => a.correct).length}/{p.answers.length} correct
                          </p>
                        </div>
                      </div>
                ...
                players.map((p, i) => {
                  const isMe = p.uid === user.uid
                  return (
                    <div key={p.uid} className={`rounded-xl border-2 p-4 ${i === 0 ? "border-primary/40 bg-primary/5" : "border-primary/5 bg-surface-card"}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 min-w-0">
                          <span className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${i === 0 ? "bg-primary text-on-primary" : "bg-surface-elevated text-canvas-soft/40"}`}>{i + 1}</span>
                          <p className="text-body-md-strong text-canvas-soft truncate">{p.name} {isMe && <span className="text-caption text-primary">(you)</span>}</p>
                        </div>
                        <p className="text-display-xs text-primary font-black">{p.score}</p>
                      </div>
                    </div>
            ...
            <div className="flex gap-3">
              <Link href="/battle">
                <Button variant="secondary" className="bg-surface-elevated text-canvas-soft hover:bg-surface-card cursor-pointer">
                  New Battle
                </Button>
              </Link>
              <Link href="/profile">
